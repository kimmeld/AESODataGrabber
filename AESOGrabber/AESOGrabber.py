...
with engine.connect() as conn:
    while True:
        ...
        # Fetch summary data
        resp = requests.get("https://api.aeso.ca/report/v1/csd/summary/current", headers={"X-API-Key": AESO_API_KEY})
        jsondata = resp.json()
        ts = datetime.datetime.strptime(jsondata['return']['last_updated_datetime_utc'], '%Y-%m-%d %H:%M').replace(tzinfo=datetime.timezone.utc)
        with conn.begin():
            try:
                ...
                conn.execute(text("insert into aeso_summary(ts, total_max_generation_capability, total_net_generation, net_to_grid_generation, net_actual_interchange, "
                                  ...
                                  ":dispatched_contingency_reserve_other, :lssi_armed_dispatch, :lssi_offered_volume)"), data)
                
                data = []
                for gendata in jsondata['return']['generation_data_list']:
                    gendata['ts'] = ts
                    data.append(gendata)
                conn.execute(text("insert into aeso_summary_generation(ts, fuel_type, aggregated_maximum_capability, aggregated_net_generation, aggregated_dispatched_contingency_reserve) "
                                  "values (:ts, :fuel_type, :aggregated_maximum_capability, :aggregated_net_generation, :aggregated_dispatched_contingency_reserve)"), data)

                data = []
                for interchange in jsondata['return']['interchange_list']:
                    interchange['ts'] = ts
                    data.append(interchange)
                conn.execute(text("insert into aeso_summary_interchange(ts, flow_path, actual_flow) values (:ts, :path, :actual_flow)"), data)
